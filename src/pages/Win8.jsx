...
export default function Win8() {
  ...
  const { bar, filteredGames } = useCategoryAndSort(games, {
    labels: { casino: 'Casino', slots: 'Slots', card: 'Card', arcade: 'Arcade' },
  })
  ...
  useEffect(() => {
    const loadGames = async () => {
      ...
      try {
        ...
        console.log('[3win8] Loaded:', result?.length || 0, '| image pool:', imagePool.length)
        if (result && result.length > 0) setGames(result)
      } catch (e) {
        console.error('[3win8] Error:', e)
      }
      setLoading(false)
    }
    loadGames()
    ...
    ;(async () => {
      if (user?.accountId) {
        try {
          ...
          console.error('[3win8] Exit error:', error)
        }
        try {
          const result = await walletService.getBalance(user.accountId)
          if (result.success && result.balance !== undefined) {
  ...
  const handlePlayNow = async (game, e) => {
    ...
    recordLaunch(ProviderKey.WIN8, user?.accountId, { preLaunchBalance: preBalance })

    const mainBalance = Number(user?.balance) || 0
    const amount = Math.min(DEFAULT_LAUNCH_AMOUNT, Math.floor(mainBalance))

    try {
      const result = await launchWin8Game(game, user?.accountId, {
      ...
      if (result.success && result.gameUrl) {
        ...
        if (result.errorCode === 'INSUFFICIENT_USER_FUND') {
          ...
          showToast('Your 3win8 account is suspended.', 'error')
        } else if (result.errorCode === 'PLAY_GAME_FAIL') {
          showToast('This game is not available right now.', 'warning')
        } else if (result.state === 'DEPOSIT_FAILED') {
          // Deposit went through but URL failed — sweep chips back
          showToast('Game launch failed — refunding your balance.', 'error')
          try { await exitWin8Game(user.accountId) } catch { /* best-effort */ }
        } else {
          showToast(result.error || '3win8 temporarily unavailable.', 'error')
        }
      }
    } catch (error) {
      console.error('[3win8 Launch] error:', error)
    ...
    <div className="slot-page">
      <div className="marquee">
        ...
        <div className="marquee-text">
          <span>Telegram: @Team33 | 3win8 Casino (Transfer Wallet)</span>
        </div>
      </div>
      ...
      <div className="slot-content">
        ...
        <div className="games-count">
          {filteredGames.length} 3win8 games available
        </div>
        ...
        {loading ? (
          ...
          <div className="slot-games-layout">
            {filteredGames.length > 0 ? (
              <div className="game-category-section win8-section">
              ...
              <div className="empty-state">
                <p>No 3win8 games available right now.</p>
              </div>
            )}
          </div>
        )}
